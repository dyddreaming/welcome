        ...
        <el-table :data="tableData" border style="width: 100%">
          <el-table-column label="任务名称" width="auto" align="center">
            <template slot-scope="scope">
              <div style="display: flex; margin-left: 30px; position: relative">
                <!-- 任务图片 -->
                <img
                  :src="scope.row.img"
          ...
          <el-table-column label="已完成人数" width="auto" align="center">
            <template slot-scope="scope">
              <el-progress
                :percentage="makePercent(scope.row.number, totalNumber)"
                :color="customColor"
              ></el-progress>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    makePercent(number, total) {
      return total === 0 ? 0 : Math.floor((number / total) * 100);
    },
    loadData() {
      // 每次请求前进行清空
      this.tableData = [];
      ...
      axios
        ...
        .then((response) => {
          this.totalNumber = response.data.data.registerCount;
