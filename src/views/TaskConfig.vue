  ...
  <div class="app-main" style="position: relative; background-color: #f0f3f4">
    ...
    <div style="
      ...
      <div style="
        ...
        <el-button type="primary" style="background-color: #e85656; border: #e85656" size="small"
          @click="taskDelete">批量删除</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(`${this.$store.getters.getIp}/administrators/${createUser}`)
      .then((response) => {
        const name = response.data.data.name; // 获取到发布者姓名
        resolve(name); // 将获取到的姓名作为resolve的参数传递
      })
      .catch((error) => {
        reject(error);
  ...
  try {
    const response = await axios.get(`${this.$store.getters.getIp}/tasks/page${queryString}`);
    this.totalData = response.data.data.records;
    console.log("获取到的任务列表:", this.totalData);
    this.totalPages = response.data.data.pages;
    this.tableData = await Promise.all(this.totalData.map(async (item) => {
      let target = "";
      switch (item.target) {
        case 0:
          target = "本科生";
      ...
      // 获取任务发布者
      const createUserName = await this.getName(item.createUser);
